...
function cargarPro2(idC){
    ...
    try {
        $.ajax({
            ...
            try {
                console.log(item);
                document.getElementById("IdProductoT").value = item.codigo;
                document.getElementById("IdProductoT").disabled = true;
                document.getElementById("NombreProducto").value = item.nombre;
                document.getElementById("NombreProducto").disabled = true;
                document.getElementById("CantidadEx").value = item.cantidad;
                document.getElementById("CantidadMo").value = 1;
                document.getElementById("CantidadMo").setAttribute("max",item.cantidad);
                a=document.getElementsByTagName("a");
                for (i = 0; i < a.length; i++) {
                    txtValue = a[i].textContent || a[i].innerText;
                    a[i].style.display = "none";
...
try {
    $.ajax({
        ...
        $.each(response, function (i, item) {
        try {
            A  = '<a class="Aelemente" href="javascript:cargarPro2('+item.codigo+')"> <i class="fas fa-cubes" style="float: left; font-size: 21px;margin-left:5px ; margin-right:13px ; " ></i>'+item.nombre+' <p class="dato2" >'+item.codigo+' </p> </a>'
            console.log(A);
            $("#dropdownPro").append(A);
